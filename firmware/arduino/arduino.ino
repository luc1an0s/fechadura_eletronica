...
// Configuração do LCD I2C (Endereço comum é 0x27 ou 0x3F)
...
void loop() {
  ...
  switch (estadoAtual) {
    ...
    case ESPERA:
      if (key) {
        // Envia para o PC saber que começou
        if (isDigit(key)) {
           Serial.print(key); // <--- IMPORTANTE: Envia o primeiro dígito pro PC
           entradaID += key;
        } else if (key == '*') {
           Serial.print(key); // <--- IMPORTANTE: Envia comando de limpar pro PC
        }
    ...
    case DIGITANDO_ID:
      if (key) {
        Serial.print(key); // <--- CORREÇÃO: Envia TUDO que for digitado nesta tela para o PC
...
void validarLogin(String id, String senha) {
  for (int i = 0; i < totalUsuarios; i++) {
    ...
    if (usuarios[i].id == id && usuarios[i].senha == senha) {
      ...
      String timestamp = String(millis() / 1000);
      // O println envia uma linha nova, o que é bom para LOGs, mas não confunde a digitação pois o PC filtra logs
...
void cadastrarUsuarioRFID(String comando) {
  ...
  for (int i = 0; i < totalUsuarios; i++) {
    if (usuarios[i].id == id) {
      ...
      usuarios[i].nivel = nivel;
      usuarios[i].uid = uid;
      lcd.clear(); lcd.print("User Atualiz.");
      delay(1000); resetarEstado();
      return;
    }
  }
  ...
  totalUsuarios++;
  lcd.clear(); lcd.print("User Criado");
  delay(1000); resetarEstado();
}
